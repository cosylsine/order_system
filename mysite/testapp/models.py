# This is an auto-generated Django model module.
# You'll have to do the following manually to clean this up:
#   * Rearrange models' order
#   * Make sure each model has one field with primary_key=True
#   * Make sure each ForeignKey and OneToOneField has `on_delete` set to the desired behavior
#   * Remove `managed = False` lines if you wish to allow Django to create, modify, and delete the table
# Feel free to rename the models, but don't rename db_table values or field names.
from django.db import models


class Addr(models.Model):
    addr_id = models.CharField(primary_key=True, max_length=10)
    area = models.CharField(max_length=10)
    building = models.CharField(max_length=10)
    room = models.CharField(max_length=10)

    class Meta:
        managed = False
        db_table = 'addr'


class AuthGroup(models.Model):
    name = models.CharField(unique=True, max_length=150)

    class Meta:
        managed = False
        db_table = 'auth_group'


class AuthGroupPermissions(models.Model):
    id = models.BigAutoField(primary_key=True)
    group = models.ForeignKey(AuthGroup, models.DO_NOTHING)
    permission = models.ForeignKey('AuthPermission', models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'auth_group_permissions'
        unique_together = (('group', 'permission'),)


class AuthPermission(models.Model):
    name = models.CharField(max_length=255)
    content_type = models.ForeignKey('DjangoContentType', models.DO_NOTHING)
    codename = models.CharField(max_length=100)

    class Meta:
        managed = False
        db_table = 'auth_permission'
        unique_together = (('content_type', 'codename'),)


class AuthUser(models.Model):
    password = models.CharField(max_length=128)
    last_login = models.DateTimeField(blank=True, null=True)
    is_superuser = models.IntegerField()
    username = models.CharField(unique=True, max_length=150)
    first_name = models.CharField(max_length=150)
    last_name = models.CharField(max_length=150)
    email = models.CharField(max_length=254)
    is_staff = models.IntegerField()
    is_active = models.IntegerField()
    date_joined = models.DateTimeField()

    class Meta:
        managed = False
        db_table = 'auth_user'


class AuthUserGroups(models.Model):
    id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey(AuthUser, models.DO_NOTHING)
    group = models.ForeignKey(AuthGroup, models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'auth_user_groups'
        unique_together = (('user', 'group'),)


class AuthUserUserPermissions(models.Model):
    id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey(AuthUser, models.DO_NOTHING)
    permission = models.ForeignKey(AuthPermission, models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'auth_user_user_permissions'
        unique_together = (('user', 'permission'),)


class Canteen(models.Model):
    canteen_id = models.CharField(primary_key=True, max_length=10)
    canteen_name = models.CharField(max_length=5)

    class Meta:
        managed = False
        db_table = 'canteen'


class CanteenManager(models.Model):
    manager_id = models.CharField(primary_key=True, max_length=10)
    can_canteen = models.ForeignKey(Canteen, models.DO_NOTHING)
    manager_name = models.CharField(max_length=5)
    manager_tel = models.CharField(max_length=11)
    canteen_id = models.CharField(max_length=2)
    manager_pwd = models.CharField(max_length=16)

    class Meta:
        managed = False
        db_table = 'canteen_manager'


class Dish(models.Model):
    dish_id = models.CharField(primary_key=True, max_length=10)
    seller = models.ForeignKey('Seller', models.DO_NOTHING)
    dish_name = models.CharField(max_length=16)
    dish_price = models.IntegerField()
    dish_description = models.CharField(max_length=100, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'dish'


class DjangoAdminLog(models.Model):
    action_time = models.DateTimeField()
    object_id = models.TextField(blank=True, null=True)
    object_repr = models.CharField(max_length=200)
    action_flag = models.PositiveSmallIntegerField()
    change_message = models.TextField()
    content_type = models.ForeignKey('DjangoContentType', models.DO_NOTHING, blank=True, null=True)
    user = models.ForeignKey(AuthUser, models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'django_admin_log'


class DjangoContentType(models.Model):
    app_label = models.CharField(max_length=100)
    model = models.CharField(max_length=100)

    class Meta:
        managed = False
        db_table = 'django_content_type'
        unique_together = (('app_label', 'model'),)


class DjangoMigrations(models.Model):
    id = models.BigAutoField(primary_key=True)
    app = models.CharField(max_length=255)
    name = models.CharField(max_length=255)
    applied = models.DateTimeField()

    class Meta:
        managed = False
        db_table = 'django_migrations'


class DjangoSession(models.Model):
    session_key = models.CharField(primary_key=True, max_length=40)
    session_data = models.TextField()
    expire_date = models.DateTimeField()

    class Meta:
        managed = False
        db_table = 'django_session'


class Guest(models.Model):
    guest_id = models.CharField(primary_key=True, max_length=10)
    guest_name = models.CharField(max_length=20)
    addr = models.ForeignKey(Addr, models.DO_NOTHING)
    guest_tel = models.CharField(max_length=11)
    guest_pwd = models.CharField(max_length=16)

    class Meta:
        managed = False
        db_table = 'guest'


class Orders(models.Model):
    order_id = models.CharField(primary_key=True, max_length=10)
    dish = models.ForeignKey(Dish, models.DO_NOTHING)
    guest = models.ForeignKey(Guest, models.DO_NOTHING)
    order_dish_num = models.IntegerField()
    order_time = models.CharField(max_length=20)
    order_amount = models.IntegerField()
    order_status = models.CharField(max_length=5)

    class Meta:
        managed = False
        db_table = 'orders'


class Review(models.Model):
    review_id = models.CharField(primary_key=True, max_length=10)
    guest_id = models.CharField(max_length=10, blank=True, null=True)
    content = models.CharField(max_length=100, blank=True, null=True)
    score = models.DecimalField(max_digits=1, decimal_places=0)
    order = models.ForeignKey(Orders, models.DO_NOTHING)

    class Meta:
        managed = False
        db_table = 'review'


class Seller(models.Model):
    seller_id = models.CharField(primary_key=True, max_length=10)
    canteen = models.ForeignKey(Canteen, models.DO_NOTHING)
    seller_name = models.CharField(max_length=10)
    seller_pwd = models.CharField(max_length=16)
    seller_description = models.CharField(max_length=100, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'seller'


class WhatToEat(models.Model):
    # 定义与视图中字段对应的属性
    id = models.AutoField(primary_key=True, db_column='id')
    canteen_name = models.CharField(max_length=5)
    seller_name = models.CharField(max_length=10)
    seller_description = models.CharField(max_length=100, blank=True, null=True)
    dish_name = models.CharField(max_length=16)
    dish_price = models.IntegerField()
    dish_description = models.CharField(max_length=100, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'what_to_eat'  # 视图在数据库中的名称


class OrderDetails(models.Model):
    order_id = models.AutoField(primary_key=True, db_column='order_id')
    guest_name = models.CharField(max_length=20)
    order_time = models.CharField(max_length=20)
    seller_id = models.CharField(max_length=10)
    dish_name = models.CharField(max_length=16)
    order_dish_num = models.IntegerField()
    order_amount = models.IntegerField()
    order_status = models.CharField(max_length=5)

    class Meta:
        managed = False
        db_table = 'order_details'  # 视图在数据库中的名称